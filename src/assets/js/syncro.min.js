...
function initSteppedProgress(chartdata) {
  [].forEach.call(
    document.querySelectorAll("[chartdata]"),
    function (steppedProgress, index) {
      if (steppedProgress) {
        var valueTotal = 0;
        var data;
        if (steppedProgress.getAttribute("chartdata")) {
          data = JSON.parse(steppedProgress.getAttribute("chartdata"));
        } else {
          data = chartdata;
        }
