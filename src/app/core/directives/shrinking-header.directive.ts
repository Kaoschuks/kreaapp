...
export class ShrinkingHeaderDirective implements OnInit {
  @Input('shrinkingHeader') scrollArea: any;
  newHeaderHeight: any;
  ...
  initStyles() {
    ...
    this.scrollArea.ionScroll.subscribe((scrollEvent: any) => {
      this.resizeHeader(scrollEvent);
    });
  }

  resizeHeader(scrollEvent: any) {
    this.domCtrl.write(() => {
  ...
  changeHeaderContent(bgOpacityPercent: any, navOpacityPercent: any) {
